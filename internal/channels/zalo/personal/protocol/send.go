...
func buildMultipartBody(fieldName, fileName string, data []byte) (io.Reader, string, error) {
	...
	// Use a fixed boundary for determinism (Zalo doesn't care about boundary format)
...
// pngDimensions extracts width and height from a PNG file header.
// Returns (0, 0) for non-PNG files.
