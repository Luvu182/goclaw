...
import { Upload, CheckCircle2, XCircle, Loader2, X, TriangleAlert } from "lucide-react";
import { DialogFooter } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { cn, uniqueId } from "@/lib/utils";
import { validateSkillZip } from "./lib/validate-skill-zip";
import type { FileEntry, FileStatus } from "./skill-install-types";
...
export function FileTab({ onUpload, onClose, tabSwitcher }: FileTabProps) {
  ...
  return (
    <>
      {!uploading && !fileDone && tabSwitcher}
      ...
      <DialogFooter>
        ...
        {fileDone ? (
          <Button onClick={onClose}>{t("upload.done")}</Button>
        ) : (
